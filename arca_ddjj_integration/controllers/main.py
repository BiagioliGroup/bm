...
class LibroIVAController(http.Controller):
    ...
    def descargar_libro_iva(self, **kwargs):
        ...
        sheet.write(row, 6, sum(c.importe_neto for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 7, sum(c.iva for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 8, 0, bold_money)  # Neto 10.5%
        sheet.write(row, 9, 0, bold_money)  # IVA 10.5%
        sheet.write(row, 10, sum(c.perc_iva for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 11, sum(c.perc_tem for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 12, sum(c.perc_iibb for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 13, sum(c.imp_internos for c in comprobantes if c.incluir_en_ddjj), bold_money)
        sheet.write(row, 14, 0, bold_money)
        sheet.write(row, 15, sum(c.importe_total for c in comprobantes if c.incluir_en_ddjj), bold_money)
